...
int main() {
	...
	if (result_1 == 1) {
		printf("3-(a)번의 수행결과,두 행렬이 같음이 입증되었습니다.");
	}
	else {
		printf("두행렬이 다릅니다.");
	}
	...
	//우변 인수
		//첫번째 항
	double** f1_Hlt_Hl = multipleMatrix_DoubleDouble(Hlt, Hl, n, n / 2, n);
	double** f1_Hlt_Hl_A = multipleMatrix_DoubleInt(f1_Hlt_Hl, A, n, n, n);
	double** f1_Hlt_Hl_A_Hlt = multipleMatrix_DoubleDouble(f1_Hlt_Hl_A, Hlt, n, n, n / 2);
	double** f1_Hlt_Hl_A_Hlt_Hl = multipleMatrix_DoubleDouble(f1_Hlt_Hl_A_Hlt, Hl, n, n / 2, n);
	double** firstTermMatrix = f1_Hlt_Hl_A_Hlt_Hl;
	//두번째 항
	double** f2_Hlt_Hl = multipleMatrix_DoubleDouble(Hlt, Hl, n, n / 2, n);
	double** f2_Hlt_Hl_A = multipleMatrix_DoubleInt(f2_Hlt_Hl, A, n, n, n);
	double** f2_Hlt_Hl_A_Hht = multipleMatrix_DoubleDouble(f2_Hlt_Hl_A, Hht, n, n, n / 2);
	double** f2_Hlt_Hl_A_Hht_Hh = multipleMatrix_DoubleDouble(f2_Hlt_Hl_A_Hht, Hh, n, n / 2, n);
	double** secondTermMatrix = f2_Hlt_Hl_A_Hht_Hh;
	//세번째 항
	double** f3_Hht_Hh = multipleMatrix_DoubleDouble(Hht, Hh, n, n / 2, n);
	double** f3_Hht_Hh_A = multipleMatrix_DoubleInt(f3_Hht_Hh, A, n, n, n);
	double** f3_Hht_Hh_A_Hlt = multipleMatrix_DoubleDouble(f3_Hht_Hh_A, Hlt, n, n, n / 2);
	double** f3_Hht_Hh_A_Hlt_Hl = multipleMatrix_DoubleDouble(f3_Hht_Hh_A_Hlt, Hl, n, n / 2, n);
	double** thirdTermMatrix = f3_Hht_Hh_A_Hlt_Hl;
	//네번째 항
	double** f4_Hht_Hh = multipleMatrix_DoubleDouble(Hht, Hh, n, n / 2, n);
	double** f4_Hht_Hh_A = multipleMatrix_DoubleInt(f4_Hht_Hh, A, n, n, n);
	double** f4_Hht_Hh_A_Hht = multipleMatrix_DoubleDouble(f4_Hht_Hh_A, Hht, n, n, n / 2);
	double** f4_Hht_Hh_A_Hht_Hh = multipleMatrix_DoubleDouble(f4_Hht_Hh_A_Hht, Hh, n, n / 2, n);
	double** fourthTermMatrix = f4_Hht_Hh_A_Hht_Hh;
	// 우변인수 덧셈
	double** first_second = sumMatrix_DoubleDouble(firstTermMatrix, secondTermMatrix, n);
	double** frist_second_third = sumMatrix_DoubleDouble(first_second, thirdTermMatrix, n);
	double** first_seond_third_fourth = sumMatrix_DoubleDouble(frist_second_third, fourthTermMatrix, n);
	double** rightTermOfb = first_seond_third_fourth;
	//같나 테스트
	int result_2 = 1;
	for (int i = 0; i < n; i++) {
		for (int j = 0; j < n; j++) {
			if (abs(leftMatrixOfb[i][j] - rightTermOfb[i][j]) > 0.0001) {
				result_2 = 0;
			}
		}
	}
	if (result_2 == 1) {
		printf("3-(b)번의 수행결과,두 행렬이 같음이 입증되었습니다.");
	}
	else {
		printf("두행렬이 다릅니다.");
	}
	...
	writeBitmapFile(bytesPerPixel, outputHeader, Are_1, imgSize, "changed_cloud_1.bmp");
	writeBitmapFile(bytesPerPixel, outputHeader, Are_2, imgSize, "changed_cloud_2.bmp");
	writeBitmapFile(bytesPerPixel, outputHeader, Are_3, imgSize, "changed_cloud_3.bmp");
	writeBitmapFile(bytesPerPixel, outputHeader, Are_4, imgSize, "changed_cloud_4.bmp");
